...
import { ArrowLeft, Plus, Phone, Mail, Calendar, Send, Trash2, Download, Edit, Key, FileText, Bell } from 'lucide-react';
...
const PatientDetail = () => {
  ...
  const handleMarkReady = async (bilanId) => {
    try { await markReady.mutateAsync(bilanId); toast.success('Notifications en cours d\'envoi'); }
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      if (res.generatedPassword) setRevealed({ password: res.generatedPassword, credentials: res.credentials });
      ...
      <div className="glass rounded-3xl p-8 mb-8 shadow-soft animate-fade-rise">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-6">
          <div className="flex items-start gap-5">
            <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-mint-200 to-mint-400 flex items-center justify-center text-paper text-3xl display shadow-soft flex-shrink-0">
              {patient.full_name.charAt(0).toUpperCase()}
            </div>
            <div>
              <h1 className="display text-4xl text-ink">{patient.full_name}</h1>
              <div className="flex flex-wrap gap-x-5 gap-y-2 mt-3 text-sm text-muted">
                <span className="inline-flex items-center gap-1.5"><Phone className="w-3.5 h-3.5" /><span className="mono">{patient.phone}</span></span>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted mt-2 mono">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    {b.notification_sent && (
                      <button
                        ...
                        className="p-2.5 rounded-xl hover:bg-amber-50 text-muted hover:text-amber-500 transition"
                      >
